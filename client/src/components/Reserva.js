    ...
    const fetchBookedTimes = async () => {
      if (selectedDate) {
        try {
          ...
          const response = await fetch(`http://localhost:3002/api/availability?date=${formattedDate}`);
          if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const formattedDate = selectedDate.format('YYYY-MM-DD');
        const updatedResponse = await fetch(`http://localhost:3002/api/availability?date=${formattedDate}`);
